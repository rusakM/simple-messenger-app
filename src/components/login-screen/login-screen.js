...
class LoginScreen extends Component {
  constructor(props) {
    ...
    this.formRegisterFirstPwHandler = this.formRegisterFirstPwHandler.bind(
      this
    );
    this.formRegisterSecondPwHandler = this.formRegisterSecondPwHandler.bind(
      this
    );
    this.formRegisterNameHandler = this.formRegisterNameHandler.bind(this);
    this.formRegisterSurnameHandler = this.formRegisterSurnameHandler.bind(
      this
    );
  ...
  handleLogin = event => {
    ...
    fetch(`${links.api}/login`, {
      ...
      headers,
      body: `email=${email}&password=${md5(password)}`
    })
  ...
  handleRegister(event) {
    event.preventDefault();
    if (
      this.state.registerForm.password.first !==
      this.state.registerForm.password.second
    ) {
      this.setState({
        alertPanel: {
          classes: "alert-panel alert-panel-show",
          ...
          case 0:
            this.setState({
              alertPanel: {
                classes: "alert-panel alert-panel-show",
                message:
                  "An account already exists with your address email. Try again"
              }
            });
            break;
          case 1:
            this.setState({
              alertPanel: {
                classes: "alert-panel alert-panel-show",
                message:
                  "You will receive email message to verify your account. Check youe mailbox"
              }
            });
            break;
          default:
  ...
  render() {
    return (
      <div className="login-screen">
        <header className="header-login-screen">@rusio chat app</header>
        <div className="buttons">
          <button
            onClick={this.enableFormLogin}
            className={this.state.buttons.left}
          >
            Login
          </button>
          <button
            onClick={this.enableRegisterLogin}
            className={this.state.buttons.right}
          >
            Register
          </button>
        </div>
...
const FormLogin = props => {
  return (
    <form onSubmit={props.handleLogin}>
      <input
        onChange={props.formLoginEmailHandler}
      ...
      <input
        type="password"
        onChange={props.formLoginPwHandler}
        className="input-field"
...
const FormRegister = props => {
  return (
    <form onSubmit={props.handleRegister}>
      <input
        onChange={props.formRegisterEmailHandler}
        className="input-field"
        value={props.values.email}
        placeholder="Email"
        required
      ...
      <input
        type="password"
        onChange={props.formRegisterFirstPwHandler}
        className="input-field"
        value={props.values.password.first}
        placeholder="Password"
        required
      />
      <input
        type="password"
        onChange={props.formRegisterSecondPwHandler}
        className="input-field"
        value={props.values.password.second}
        placeholder="Repeat password"
        required
      />
      <input
        onChange={props.formRegisterNameHandler}
        className="input-field"
      ...
      />
      <input
        onChange={props.formRegisterSurnameHandler}
        className="input-field"
